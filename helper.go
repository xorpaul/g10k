...
func executeCommand(command string, commandDir string, timeout int, allowFail bool, disableHTTPProxy bool) ExecResult {
	...
	if (allowFail || GlobalConfig.UseCacheFallback) && err != nil {
...
func checkForAndExecutePostrunCommand() {
	if len(GlobalConfig.PostRunCommand) > 0 {
		postrunCommandString := strings.Join(GlobalConfig.PostRunCommand, " ")
		...
		er := executeCommand(postrunCommandString, "", GlobalConfig.Timeout, false, false)
