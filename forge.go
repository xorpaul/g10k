...
func checkDeprecation(fm ForgeModule, lastCheckedFile string) bool {
	...
	if fileInfo, err := os.Stat(lastCheckedFile); err == nil {
		if fileInfo.Size() < 1 {
			logging.Debugf("found empty file " + lastCheckedFile)
			return false
			...
			if err != nil {
				logging.Fatalf("doModuleInstallOrNothing(): Error while reading Forge API result from file " + lastCheckedFile + err.Error())
			}
...
func doModuleInstallOrNothing(fm ForgeModule) {
	...
	workDir := filepath.Join(GlobalConfig.ForgeCacheDir, moduleName+"-"+fm.version)
	lastCheckedFile := filepath.Join(GlobalConfig.ForgeCacheDir, moduleName+"-latest-last-checked")
	...
	if moduleVersion == "latest" {
		if !fsutils.IsDir(workDir) {
			...
			if fr.needToGet {
				if _, ok := uniqueForgeModules[moduleName+"-"+fr.versionNumber]; ok {
					logging.Debugf("no need to fetch Forge module " + moduleName + " in latest, because latest is " + fr.versionNumber + " and that will already be fetched")
					fr.needToGet = false
					versionDir := filepath.Join(GlobalConfig.ForgeCacheDir, moduleName+"-"+fr.versionNumber)
					absolutePath, err := filepath.Abs(versionDir)
					logging.Debugf("trying to create symlink " + workDir + " pointing to " + absolutePath)
					if err != nil {
						logging.Fatalf("doModuleInstallOrNothing(): Error while resolving absolute file path for " + versionDir + " Error: " + err.Error())
					}
					if err := os.Symlink(absolutePath, workDir); err != nil {
						logging.Fatalf("doModuleInstallOrNothing(): 1 Error while creating symlink " + workDir + " pointing to " + absolutePath + " Error: " + err.Error())
					}
					//} else {
					//logging.Debugf("need to fetch Forge module " + moduleName + " in latest, because version " + fr.versionNumber + " will not be fetched already")
			...
			if fm.cacheTTL > 0 {
				//logging.Debugf("checking for " + lastCheckedFile)
				if fileInfo, err := os.Lstat(lastCheckedFile); err == nil {
					//logging.Debugf("found " + lastCheckedFile + " with mTime " + fileInfo.ModTime().String())
					if fileInfo.ModTime().Add(fm.cacheTTL).After(time.Now()) {
						logging.Debugf("No need to check forge API if latest version of module " + moduleName + " has been updated, because last-checked file " + lastCheckedFile + " is not older than " + fm.cacheTTL.String())
			...
			logging.Debugf("check forge API if latest version of module " + moduleName + " has been updated")
		...
		latestDir := filepath.Join(GlobalConfig.ForgeCacheDir, moduleName+"-latest")
		if !fsutils.IsDir(latestDir) {
			...
			logging.Debugf("we got " + fm.author + "-" + fm.name + "-" + fm.version + ", but no " + latestDir + " to use. Getting -latest")
			fm.version = "latest"
		...
		if !fsutils.IsDir(workDir) {
			...
			logging.Debugf("Using cache for " + moduleName + " in version " + moduleVersion + " because " + workDir + " exists")
			return
		}
	}
	...
	if fr.needToGet {
		if fm.version != "latest" {
			logging.Debugf("Trying to remove: " + workDir)
			_ = os.Remove(workDir)
		} else {
			// os.Readlink error is okay
			versionDir, _ := os.Readlink(workDir)
			if versionDir == filepath.Join(GlobalConfig.ForgeCacheDir, moduleName+"-"+fr.versionNumber) {
				...
				logging.Debugf("trying to create symlink " + workDir + " pointing to " + absolutePath)
				if err := os.Symlink(absolutePath, workDir); err != nil {
					logging.Fatalf("doModuleInstallOrNothing(): 2 Error while creating symlink " + workDir + " pointing to " + absolutePath + err.Error())
				}
			}
		}
...
func queryForgeAPI(fm ForgeModule) ForgeResult {
	baseURL := GlobalConfig.ForgeBaseURL
	if len(fm.baseURL) > 0 {
		baseURL = fm.baseURL
	}
	...
	if err != nil {
		logging.Fatalf("queryForgeAPI(): Error creating GET request for Puppetlabs forge API" + err.Error())
	}
	...
	if err != nil {
		logging.Fatalf("queryForgeAPI(): Error while getting http proxy with golang http.ProxyFromEnvironment()" + err.Error())
	}
	...
	if err != nil {
		if GlobalConfig.UseCacheFallback {
			logging.Warnf("Forge API error, trying to use cache for module " + fm.author + "/" + fm.author + "-" + fm.name)
			_ = getLatestCachedModule(fm)
			return ForgeResult{false, "", "", 0}
		}
		logging.Fatalf("queryForgeAPI(): Error while issuing the HTTP request to " + url + " Error: " + err.Error())
	}
	duration := time.Since(before).Seconds()
	logging.Verbosef("Querying Forge API " + url + " took " + strconv.FormatFloat(duration, 'f', 5, 64) + "s")
	...
	if resp.StatusCode == http.StatusOK {
		...
		if err != nil {
			logging.Fatalf("queryForgeAPI(): Error while reading response body for Forge module " + fm.name + " from " + url + ": " + err.Error())
		}

		json := string(body)
		fr := parseForgeAPIResult(json, fm)

		lastCheckedFile := filepath.Join(GlobalConfig.ForgeCacheDir, fm.author+"-"+fm.name+"-latest-last-checked")
		logging.Debugf("writing last-checked file " + lastCheckedFile)
		...
		logging.Debugf("Got 304 nothing to do for module " + fm.author + "-" + fm.name)
		return ForgeResult{false, "", "", 0}
	} else if resp.StatusCode == http.StatusNotFound {
		logging.Fatalf("Received 404 from Forge for module " + fm.author + "-" + fm.name + " using URL " + url + " Does the module really exist and is it correctly named?")
		return ForgeResult{false, "", "", 0}
	}
	logging.Fatalf("Unexpected response code " + resp.Status)
	return ForgeResult{false, "", "", 0}
}
...
func parseForgeAPIResult(json string, fm ForgeModule) ForgeResult {
	...
	if deprecatedTimestamp.Exists() && deprecatedTimestamp.Value() != nil {
		...
		if logging.Info || logging.Debug {
			logging.Warnf("WARN: Forge module " + fm.author + "-" + fm.name + " has been deprecated by its author since " + deprecatedTimestamp.String() + supersededText)
		} else {
			mutex.Lock()
	...
	if len(version) < 1 {
		logging.Fatalf("ERROR: could not determine version of module " + fm.author + "/" + fm.name)
	}

	logging.Debugf("found version " + version + " for " + fm.name + "-latest")
	latestForgeModules.Lock()
...
func getMetadataForgeModule(fm ForgeModule) ForgeModule {
	...
	req, err := http.NewRequest("GET", url, nil)
	if err != nil {
		logging.Fatalf("getMetadataForgeModule(): Error while creating GET http request with url " + url + " Error: " + err.Error())
	}
	req.Header.Set("User-Agent", "https://github.com/xorpaul/g10k/")
	req.Header.Set("Connection", "keep-alive")
	proxyURL, err := http.ProxyFromEnvironment(req)
	if err != nil {
		logging.Fatalf("getMetadataForgeModule(): Error while getting http proxy with golang http.ProxyFromEnvironment()" + err.Error())
	}
	client := &http.Client{Transport: &http.Transport{Proxy: http.ProxyURL(proxyURL)}}
	before := time.Now()
	logging.Debugf("GETing " + url)
	resp, err := client.Do(req)
	duration := time.Since(before).Seconds()
	logging.Verbosef("GETing Forge metadata from " + url + " took " + strconv.FormatFloat(duration, 'f', 5, 64) + "s")
	mutex.Lock()
	syncForgeTime += duration
	mutex.Unlock()
	if err != nil {
		logging.Fatalf("getMetadataForgeModule(): Error while querying metadata for Forge module " + fm.name + " from " + url + ": " + err.Error())
	}
	defer resp.Body.Close()
	...
	if resp.StatusCode == http.StatusOK {
		...
		if err != nil {
			logging.Fatalf("getMetadataForgeModule(): Error while reading response body for Forge module " + fm.name + " from " + url + ": " + err.Error())
		}
		...
		modulemd5sum := currentRelease["file_md5"].String()
		moduleFilesize := currentRelease["file_size"].Int()
		logging.Debugf("module: " + fm.author + "/" + fm.name + " modulemd5sum: " + modulemd5sum + " moduleFilesize: " + strconv.FormatInt(moduleFilesize, 10))
	...
	logging.Fatalf("getMetadataForgeModule(): Unexpected response code while GETing " + url + " " + resp.Status)
	return ForgeModule{}
}
...
func extractForgeModule(wgForgeModule *sync.WaitGroup, file *io.PipeReader, fileName string) {
	...
	fsutils.UnTar(fileReader, GlobalConfig.ForgeCacheDir, GlobalConfig.ForgeCacheDir, GlobalConfig.PurgeSkiplist)

	if err != nil {
		logging.Fatalf(logging.FuncName() + "(): pgzip reader error for module " + fileName + " error:" + err.Error())
	}
	defer fileReader.Close()

	duration := time.Since(before).Seconds()
	logging.Verbosef("Extracting " + filepath.Join(GlobalConfig.ForgeCacheDir, fileName) + " took " + strconv.FormatFloat(duration, 'f', 5, 64) + "s")
	mutex.Lock()
...
func downloadForgeModule(name string, version string, fm ForgeModule, retryCount int) {
	...
	if !fsutils.IsDir(filepath.Join(GlobalConfig.ForgeCacheDir, name+"-"+version)) {
		baseURL := GlobalConfig.ForgeBaseURL
		if len(fm.baseURL) > 0 {
			baseURL = fm.baseURL
		}
		...
		proxyURL, err := http.ProxyFromEnvironment(req)
		if err != nil {
			logging.Fatalf(funcName + "(): Error while getting http proxy with golang http.ProxyFromEnvironment()" + err.Error())
		}
		client := &http.Client{Transport: &http.Transport{Proxy: http.ProxyURL(proxyURL)}}
		before := time.Now()
		logging.Debugf("GETing " + url)
		resp, err := client.Do(req)
		duration := time.Since(before).Seconds()
		logging.Verbosef("GETing " + url + " took " + strconv.FormatFloat(duration, 'f', 5, 64) + "s")
		mutex.Lock()
		syncForgeTime += duration
		mutex.Unlock()
		if err != nil {
			logging.Fatalf(funcName + "(): Error while GETing Forge module " + name + " from " + url + ": " + err.Error())
		}
		defer resp.Body.Close()
		...
		if resp.StatusCode == http.StatusOK {
			...
			go func() {
				defer wgForgeModule.Done()
				targetFileName := filepath.Join(GlobalConfig.ForgeCacheDir, fileName)
				logging.Debugf(funcName + "(): Trying to create " + targetFileName)
				out, err := os.Create(targetFileName)
				if err != nil {
					logging.Fatalf(funcName + "(): Error while creating file for Forge module " + targetFileName + " Error: " + err.Error())
				}
				defer out.Close()
				io.Copy(out, saveFileR)
				logging.Debugf(funcName + "(): Finished creating " + targetFileName)
			}()
			wgForgeModule.Add(1)
			...
			go func() {
				...
				if _, err := io.Copy(mw, resp.Body); err != nil {
					logging.Fatalf("Error while writing to MultiWriter " + err.Error())
				}
			}()
		} else if resp.StatusCode == http.StatusNotFound {
			logging.Fatalf("Received 404 from Forge using URL " + url +
				"\nCheck if the module name '" + fm.author + "-" + fm.name + "' and version '" + version + "' really exist" +
				"\nUsed in Puppet environment '" + fm.sourceBranch + "'")
		} else {
			logging.Fatalf("Unexpected response code while GETing " + url + " " + resp.Status)
		}
	} else {
		logging.Debugf("Using cache for Forge module " + name + " version: " + version)
	}
	wgForgeModule.Wait()
	...
	if checkSum || fm.sha256sum != "" {
		...
		if doForgeModuleIntegrityCheck(fm) {
			...
			fsutils.PurgeDir(filepath.Join(GlobalConfig.ForgeCacheDir, fileName), "downloadForgeModule()")
			fsutils.PurgeDir(strings.Replace(filepath.Join(GlobalConfig.ForgeCacheDir, fileName), ".tar.gz", "/", -1), "downloadForgeModule()")
...
func readModuleMetadata(file string) ForgeModule {
	...
	logging.Debugf("Found in file " + file + " name: " + name + " version: " + version + " author: " + author)

	moduleName := "N/A"
	if strings.Contains(name, "-") {
		moduleName = strings.Split(name, "-")[1]
	} else {
		logging.Debugf("Error: Something went wrong while decoding file " + file + " searching for the module name (found for name: " + name + "), version and author")
	}
...
func resolveForgeModules(modules map[string]ForgeModule) {
	...
	if len(modules) <= 0 {
		logging.Debugf("empty ForgeModule[] found, skipping...")
		return
	}
	...
	logging.Debugf("Resolving " + strconv.Itoa(len(modules)) + " Forge modules with " + strconv.Itoa(GlobalConfig.Maxworker) + " workers")
	...
	for m, fm := range modules {
		go func(m string, fm ForgeModule, bar *uiprogress.Bar) {
			...
			defer bar.Incr()
			defer wg.Done()
			logging.Debugf("resolveForgeModules(): Trying to get forge module " + m + " with Forge base url " + fm.baseURL + " and CacheTtl set to " + fm.cacheTTL.String())
			doModuleInstallOrNothing(fm)
			done <- true
		}(m, fm, bar)
	}
...
func check4ForgeUpdate(moduleName string, currentVersion string, latestVersion string) {
	logging.Verbosef("found currently deployed Forge module " + moduleName + " in version: " + currentVersion)
	logging.Verbosef("found latest Forge module of " + moduleName + " in version: " + latestVersion)
...
func doForgeModuleIntegrityCheck(m ForgeModule) bool {
	...
	go func(m ForgeModule) {
		...
		logging.Debugf(funcName + "(): target md5 hash sum: " + fmm.md5sum)
		if m.sha256sum != "" {
			logging.Debugf(funcName + "(): target sha256 hash sum from Puppetfile: " + m.sha256sum)
		}
	}(m)
	...
	fileName := filepath.Join(GlobalConfig.ForgeCacheDir, m.author+"-"+m.name+"-"+m.version+".tar.gz")
	...
	go func(md5R *io.PipeReader) {
		...
		if _, err := io.Copy(hashmd5, md5R); err != nil {
			logging.Fatalf(funcName + "(): Error while reading Forge module archive " + fileName + " ! Error: " + err.Error())
		}
		duration := time.Since(before).Seconds()
		logging.Verbosef("Calculating md5 sum for " + fileName + " took " + strconv.FormatFloat(duration, 'f', 5, 64) + "s")
		calculatedMd5Sum = hex.EncodeToString(hashmd5.Sum(nil))
		logging.Debugf(funcName + "(): calculated md5 hash sum: " + calculatedMd5Sum)
	}(md5R)
	...
	if m.sha256sum != "" {
		...
		go func(sha256R *io.PipeReader) {
			...
			duration := time.Since(before).Seconds()
			logging.Verbosef("Calculating sha256 sum for " + fileName + " took " + strconv.FormatFloat(duration, 'f', 5, 64) + "s")
			calculatedSha256Sum = hex.EncodeToString(hashSha256.Sum(nil))
			logging.Debugf(funcName + "(): calculated sha256 hash sum: " + calculatedSha256Sum)
		}(sha256R)
	}
	...
	go func() {
		...
		if fi, err := os.Stat(fileName); err == nil {
			...
			if err != nil {
				logging.Fatalf("Can't access Forge module archive " + fileName + " ! Error: " + err.Error())
			}
			defer file.Close()
		...
		duration := time.Since(before).Seconds()
		logging.Verbosef("Calculating hash sum(s) for " + fileName + " took " + strconv.FormatFloat(duration, 'f', 5, 64) + "s")
		logging.Debugf(funcName + "(): calculated archive size: " + strconv.FormatInt(calculatedArchiveSize, 10))
	}()

	wgCheckSum.Wait()

	if fmm.md5sum != calculatedMd5Sum {
		logging.Warnf("WARNING: calculated md5sum " + calculatedMd5Sum + " for " + fileName + " does not match expected md5sum " + fmm.md5sum)
		return true
	}
	if m.sha256sum != calculatedSha256Sum {
		logging.Warnf("WARNING: calculated sha256sum " + calculatedSha256Sum + " for " + fileName + " does not match expected sha256sum " + m.sha256sum)
		return true
	}
	if fmm.fileSize != calculatedArchiveSize {
		logging.Warnf("WARNING: calculated file size " + strconv.FormatInt(calculatedArchiveSize, 10) + " for " + fileName + " does not match expected file size " + strconv.FormatInt(fmm.fileSize, 10))
		return true
	}
	logging.Debugf("calculated file size " + strconv.FormatInt(calculatedArchiveSize, 10) + " for " + fileName + " does match expected file size " + strconv.FormatInt(fmm.fileSize, 10))
	logging.Debugf("calculated md5sum " + calculatedMd5Sum + " for " + fileName + " does match expected md5sum " + fmm.md5sum)
	if m.sha256sum != "" {
		logging.Debugf("calculated sha256sum " + calculatedSha256Sum + " for " + fileName + " does match expected sha256sum " + m.sha256sum)
	}
	return false
...
func syncForgeToModuleDir(name string, m ForgeModule, moduleDir string, correspondingPuppetEnvironment string) {
	...
	//logging.Debugf("m.name " + m.name + " m.version " + m.version + " moduleName " + moduleName)
		...
		if fsutils.FileExists(metadataFile) {
			...
			if m.version == "latest" {
				...
				if _, ok := latestForgeModules.m[moduleName]; ok {
					logging.Debugf("using version " + latestForgeModules.m[moduleName] + " for " + moduleName + "-" + m.version)
			...
			if me.version == m.version {
				logging.Debugf("Nothing to do, existing Forge module: " + targetDir + " has the same version " + me.version + " as the to be synced version: " + m.version)
				return
			}
	...
	workDir := fsutils.NormalizeDir(filepath.Join(GlobalConfig.ForgeCacheDir, moduleName+"-"+m.version))
	...
	if !internal.DryRun {
		...
		if fileInfo, err := os.Stat(targetDir); err == nil {
			...
			logging.Fatalf(funcName + "(): Error while os.Stat file " + targetDir)
		}
		if fileInfo, err := os.Stat(resolvedWorkDir); err == nil {
			...
			logging.Fatalf(funcName + "(): Error while os.Stat file " + resolvedWorkDir)
		}

		if targetDirDevice != workDirDevice && !usemove {
			logging.Fatalf("Error: Can't hardlink Forge module files over different devices. Please consider changing the cachedir setting. ForgeCachedir: " + GlobalConfig.ForgeCacheDir + " target dir: " + targetDir)
		}
		...
		destination := func(path string, info os.FileInfo, err error) error {
			if err != nil {
				logging.Fatalf(funcName + "(): Error while calling generic func() Error " + err.Error())
			}
			target, err := filepath.Rel(resolvedWorkDir, path)
			if err != nil {
				logging.Fatalf(funcName + "(): Can't make " + path + " relative to " + resolvedWorkDir + " Error: " + err.Error())
			}

			if info.IsDir() {
				if target != "." { // skip the root dir
					//logging.Debugf(funcName + "() Trying to mkdir " + filepath.Join(targetDir, target))
					err = os.Mkdir(filepath.Join(targetDir, target), os.FileMode(0755))
					if err != nil {
						logging.Fatalf(funcName + "(): error while Mkdir() " + targetDir + "/" + target + " Error: " + err.Error())
					}
				}
			} else {
				if usemove {
					...
					//logging.Debugf(funcName + "() Trying to hardlink " + path + " to " + filepath.Join(targetDir, target))
					err = os.Link(path, filepath.Join(targetDir, target))
					if err != nil {
						logging.Fatalf(funcName + "(): Failed to hardlink " + path + " to " + targetDir + "/" + target + " Error: " + err.Error())
					}
				}
			}
			return nil
		...
		c := make(chan error)
		logging.Debugf(funcName + "() filepath.Walk'ing directory " + resolvedWorkDir)
		before := time.Now()
		...
		ioForgeTime += duration
		mutex.Unlock()
		logging.Verbosef("Populating " + targetDir + " took " + strconv.FormatFloat(duration, 'f', 5, 64) + "s")
	}
}
...
func getLatestCachedModule(m ForgeModule) string {
	latest := "//"
	version := "latest"
	latestDir := filepath.Join(GlobalConfig.ForgeCacheDir, m.author+"-"+m.name+"-latest")
	if !fsutils.IsDir(latestDir) {

		globPath := filepath.Join(GlobalConfig.ForgeCacheDir, m.author+"-"+m.name+"-*")
		logging.Debugf("Glob'ing with path " + globPath)
		matches, err := filepath.Glob(globPath)
		if len(matches) == 0 {
			logging.Fatalf("Could not find any cached version for Forge module " + m.author + "-" + m.name)
		}
		logging.Debugf("found potential module versions:" + strings.Join(matches, " "))
		if err != nil {
			logging.Fatalf("Failed to glob the latest cached module with glob path " + globPath + " Error: " + err.Error())
		}
		//fmt.Println(matches)
		...
		if err != nil {
			logging.Fatalf("Error while resolving absolute file path for " + latest + " Error: " + err.Error())
		}
		logging.Debugf("trying to create symlink " + latestDir + " pointing to " + latest)
		if err := os.Symlink(absolutePath, latestDir); err != nil {
			logging.Fatalf("Error while creating symlink " + latestDir + " pointing to " + absolutePath + err.Error())
		}
		version = strings.Split(latest, m.author+"-"+m.name+"-")[1]
	} else {
		versionDir, err := os.Readlink(latestDir)
		if err != nil {
			logging.Fatalf("Error while reading symlink " + latestDir + " " + err.Error())
		}

		version = strings.Split(versionDir, m.author+"-"+m.name+"-")[1]
		latest = filepath.Join(GlobalConfig.ForgeCacheDir, m.author+"-"+m.name+"-"+version)
	}

	if latest == "//" {
		logging.Fatalf("Found no usable cache for module " + m.author + "-" + m.name)
	}
	...
	logging.Warnf("Using cached version " + version + " for " + m.author + "-" + m.name + "-latest")
